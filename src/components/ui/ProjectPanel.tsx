...
const contentMap = {
  ...
  CONTACT: {
    ...
    icon: Zap,
    socials: [
      { name: "GitHub", icon: Github, link: "https://github.com/PRANAVBIRHADE", color: "#ffffff" },
      { name: "Instagram", icon: Instagram, link: "https://www.instagram.com/code_track_/", color: "#E1306C" },
      { name: "WhatsApp", icon: MessageCircle, link: "https://wa.me/917038763252", color: "#25D366" }
    ]
  }
};

export default function ProjectPanel() {
  const { activeSection, transitionPhase } = useEngineStore(
    useShallow((state) => ({
  ...
  useEffect(() => {
    // We only set visible to true after a delay when in IDLE phase
    if (transitionPhase === 'IDLE' && activeSection !== 'HOME') {
  ...
  // The panel should be rendered only when it's IDLE and we have content.
  // When transitionPhase changes to REVERSING, shouldRender becomes false, triggering AnimatePresence exit.
  const shouldRender = activeSection !== 'HOME' && transitionPhase === 'IDLE' && isVisible && !!content && !!Icon;
            ...
            <div className="space-y-8">
              ...
              {activeSection === 'ABOUT' && (
                  <div className="grid grid-cols-2 gap-6 border-t border-cyan-500/10 pt-8">
                      {(content as any).details.map((d: any, i: number) => (
                          <motion.div 
              ...
              {activeSection === 'SKILLS' && 'skills' in content && (
                  <div className="grid gap-6">
                      {(content as any).skills.map((s: any, i: number) => (
              ...
              {activeSection === 'CONTACT' && 'socials' in content && (
                  <div className="space-y-6">
                      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                          {(content as any).socials.map((social: any, i: number) => (
                              <motion.a
                                  key={i}
                                  href={social.link}
                                  target="_blank"
